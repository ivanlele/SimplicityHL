...
macro_rules! impl_name_value_map {
    ($wrapper: ident, $module_name: expr) => {
        ...
        impl<J: Jet> From<HashMap<WitnessName, Value<J>>> for $wrapper<J> {
            fn from(value: HashMap<WitnessName, Value<J>>) -> Self {
                Self(Arc::new(value))
            }
        }

        impl<J: Jet> ParseFromStr for $wrapper<J> {
            fn parse_from_str(s: &str) -> Result<Self, RichError> {
                parse::ModuleProgram::<J>::parse_from_str(s).and_then(|x| Self::analyze(&x))
            }
        }

        impl<J: Jet> fmt::Display for $wrapper<J> {
...
pub struct WitnessValues<J: Jet>(Arc<HashMap<WitnessName, Value<J>>>);

impl<J: Jet> Default for WitnessValues<J> {
    fn default() -> Self {
...
#[derive(Clone, Debug, Eq, PartialEq)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
pub struct Arguments<J: Jet>(Arc<HashMap<WitnessName, Value<J>>>);

impl<J: Jet> Default for Arguments<J> {
    fn default() -> Self {
        Self(Arc::new(HashMap::new()))
    }
}
...
impl<J: Jet> crate::ArbitraryOfType for Arguments<J> {
...
mod tests {
    ...
    assert!(jet::eq_32(witness::A, witness::A));
        ...
        let program =
            parse::Program::<ElementsExtension>::parse_from_str(s).expect("parsing works");
        match ast::Program::<ElementsExtension>::analyze(&program).map_err(Error::from) {
    ...
    assert!(jet::is_zero_32(witness::A));
        ...
        match SatisfiedProgram::<ElementsExtension>::new(s, Arguments::default(), witness, false) {
...
fn main() {
    assert!(jet::is_zero_32(f()));
        ...
        match CompiledProgram::<ElementsExtension>::new(s, Arguments::default(), false) {
    ...
    fn missing_witness_module() {
        match WitnessValues::<ElementsExtension>::parse_from_str("") {
            Ok(v) => assert!(
                v.iter().next().is_none(),
    ...
    fn redefined_witness_module() {
        ...
        match WitnessValues::<ElementsExtension>::parse_from_str(s) {
    ...
    fn witness_to_string() {
        let witness = WitnessValues::<ElementsExtension>::from(HashMap::from([
